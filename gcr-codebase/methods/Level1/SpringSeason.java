/* Write a program SpringSeason that takes two int values month and day from the command line and prints “Its a Spring Season” otherwise prints “Not a Spring Season”. 
Hint => Spring Season is from March 20 to June 20. Write a Method to check for Spring season and return a boolean true or false 
*/

//create a class name SpreinSeason
public class SpringSeason{

	//check a spring season or not
	public boolean checkSpringSeason(int month, int day){
		
		if(month > 3 && month < 6){
			return true;
		}
		else if (month == 3 && day >= 20 || month == 6 && day <= 20){
			return true;
		}
		else{
			return false;
		}
	
	}
	
	public static void main(String [] args){
		
		//taking input using cmd line args
		int month = Integer.parseInt(args[0]);
		int day = Integer.parseInt(args[1]);
		
		SpringSeason sp = new SpringSeason();
		
		boolean isSpring = sp.checkSpringSeason(month, day);
		
		//display a result 
		if(isSpring == true){
			System.out.print("Its a Spring");
		}
		else{
			System.out.print("Not a Spring");
		}
		
	}
	
}
		
		